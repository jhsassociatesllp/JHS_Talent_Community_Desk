document.addEventListener('DOMContentLoaded', () => {
    ...
    function showPage(pageId) {
        ...
        if (pageId === 'selected') fetchSelected();
        if (pageId === 'rejected') fetchRejected();
    ...
    /* --------------------------------------------------------------
   SELECT CANDIDATE → WhatsApp + DB move
   -------------------------------------------------------------- */
    async function moveToSelected(number) {
        try {
            const res = await fetch(`${API_BASE_URL}/select-candidate/${number}`, {
                method: 'POST',
                headers: { Authorization: `Bearer ${token}` }
            });
            if (!res.ok) throw new Error((await res.json()).detail || 'Failed');
            Swal.fire({icon:'success',title:'Selected!',text:'Candidate moved to Selected.'});
        } catch (e) {
    ...
    /* --------------------------------------------------------------
    REJECT CANDIDATE → WhatsApp + DB move
    -------------------------------------------------------------- */
    async function moveToRejected(number, reason) {
        try {
            const res = await fetch(`${API_BASE_URL}/reject-candidate/${number}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
            ...
            if (!res.ok) throw new Error((await res.json()).detail || 'Failed');
            Swal.fire({icon:'success',title:'Rejected',text:'Candidate moved to Rejected.'});
        } catch (e) {
            Swal.fire({icon:'error',title:'Error',text:e.message});
        }
    }
    ...
    async function fetchSelected() {
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/selected`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            if (!selected || selected.length === 0) {
                list.innerHTML = '<p class="text-gray-500">No selected candidates yet.</p>';
                return;
            }
            ...
            list.innerHTML = selected.map(c => `
                <div class="border rounded-lg p-4 bg-white flex justify-between items-center">
                    <div>
                        ...
                        Role: ${c.role}<br>
                        Phone: ${c.candidate_number}<br>
                        Date: ${c.date}<br>
                        Slot: ${c.slot}<br>
                        Status: <span class="text-green-600">Selected</span>
                    </div>
                    <div>
                        ${c.candidate_id ? `
                        <button class="icon-btn text-blue-600 hover:text-blue-800 viewResumeBtn"
                                data-cid="${c.candidate_id}"
                                title="View Resume">
            ...
            feather.replace();
            list.querySelectorAll('.viewResumeBtn').forEach(b => b.addEventListener('click', () => viewResume(b.dataset.cid)));

        } catch (e) {
            list.innerHTML = '<p class="text-red-600">Error loading selected candidates.</p>';
        }
    }
    ...
    async function fetchRejected() {
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/rejected`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (!res.ok) throw new Error('Failed');
            ...
            if (!rejected || rejected.length === 0) {
                list.innerHTML = '<p class="text-gray-500">No rejected candidates yet.</p>';
                return;
            }

            list.innerHTML = rejected.map(c => `
                <div class="border rounded-lg p-4 bg-white">
                    ...
                    Date: ${c.date}<br>
                    Slot: ${c.slot}<br>
                    Reason: <span class="text-red-600">${c.reason || '—'}</span>
                </div>
            `).join('');

        } catch (e) {
            list.innerHTML = '<p class="text-red-600">Error loading rejected candidates.</p>';
        }
    }
    ...
    /* --------------------------------------------------------------
   SCHEDULED PAGE – fetch + icons + pop-ups
   -------------------------------------------------------------- */
    async function fetchScheduled() {
        ...
        try {
            ...
            if (!scheduled || scheduled.length === 0) {
                list.innerHTML = '<p class="text-gray-500">No interviews scheduled yet.</p>';
                return;
            }

            list.innerHTML = scheduled.map(c => `
                <div class="border rounded-lg p-4 bg-white flex justify-between items-center">
                    <div>
                        <strong>${c.candidate_name}</strong><br>
                        Role: ${c.role}<br>
                        ...
                        Date: ${c.date}<br>
                        Slot: ${c.slot}<br>
                        Status: <span class="text-green-600">Scheduled</span>
                    </div>
                    <div class="flex gap-2">
                        <!-- SELECTED -->
                        <button class="icon-btn text-green-600 hover:text-green-800 selectBtn"
                                data-number="${c.candidate_number}"
                                title="Mark as Selected">
                            <i data-feather="check"></i>
                        </button>
                        <!-- REJECTED -->
                        <button class="icon-btn text-red-600 hover:text-red-800 rejectBtn"
                                data-number="${c.candidate_number}"
            ...
            // ----- SELECTED POP-UP -----
            list.querySelectorAll('.selectBtn').forEach(btn => {
                btn.addEventListener('click', async () => {
                    const number = btn.dataset.number;
                    const confirm = await Swal.fire({
                        title: 'Select Candidate',
                        text: 'Are you sure this candidate is selected?',
                        icon: 'question',
                        showCancelButton: true,
                        confirmButtonText: 'Yes',
                        cancelButtonText: 'No'
                    ...
                    await moveToSelected(number);
                    fetchScheduled();        // refresh
                    fetchSelected();         // refresh new tab
                });
            });
            ...
            list.querySelectorAll('.rejectBtn').forEach(btn => {
                btn.addEventListener('click', async () => {
                    ...
                    await moveToRejected(number, reason);
                    fetchScheduled();
                    fetchRejected();
                });
            });

            // ----- VIEW RESUME (reuse existing function) -----
            list.querySelectorAll('.viewResumeBtn').forEach(btn => {
                btn.addEventListener('click', () => viewResume(btn.dataset.cid));
            });

        } catch (e) {
            console.error(e);
            list.innerHTML = '<p class="text-red-600">Error loading scheduled interviews.</p>';
        }
    }
