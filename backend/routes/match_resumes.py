...
#     - name
#     - email
#     - phone (with 10 digit mobile number only)
#     - role
#     - score
...
#         candidate_number = candidate.get("phone")
#         # Search in messaged collection
#         messaged_doc = messaged_collection.find_one({"candidate_number": candidate_number})
#         # Search in scheduled collection
#         scheduled_doc = scheduled_collection.find_one({"candidate_number": candidate_number})
...
#     # Filter by threshold after updating all statuses
#     matched_candidates = [candidate for candidate in matched_candidates if candidate.get("score", 0) >= threshold]
...
async def match_resumes(request: MatchRequest, current_user: dict = Depends(get_current_user)):
    ...
    for candidate in matched_candidates:
        ...
        selected_doc = selected_collection.find_one({"candidate_number": candidate_number})
        # Search in rejected collection
        rejected_doc = rejected_collection.find_one({"candidate_number": candidate_number})
